...
```bash
make build   # rebuilds the image and removes the previous verifierform-stake-bet-lookup:local if replaced
...
Each `make build` replaces the previous image. If the image ID changes, `make build` removes the old image by ID — scoped to `verifierform-stake-bet-lookup:local` only, so other projects' images are unaffected.
